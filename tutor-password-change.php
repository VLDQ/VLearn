    ...
    if(strlen($_SESSION['VLearnTutorSessionCounter']) == 0) {
        ...
        if(isset($_POST['submit'])) {
            ...
            */
		    ...
		    $query = mysqli_query($con, "SELECT tutorId FROM tutors WHERE tutorId=$tutorId AND tutorPassword='$currentPassword'"); //defines query to check the correctness of current password
		    $row = mysqli_fetch_array($query); //fetches result

            if($row > 0) {
                if($newPassword != $currentPassword) {
                    $ret = mysqli_query($con, "UPDATE tutors SET tutorPassword='$newPassword' WHERE tutorId=$tutorId"); //defines query to update tutor password based on tutor id
...
<head>
	...
	<title>VLearn for Tutors | Password Change</title>
...
<body>
    ...
    <!-- JavaScript -->
    <script src="js/tutor-password-change-validation.js"></script>
